 ...
 *
 * @return {Array[Array]} The collection of results for the traversal at each depth
 */
 ...
 * </p>
 *
 * @param  focusedElement {HTMLElement} The element that has focus (or any other if you want), with which interceptors will be compared
 *
 ...
 *
 * @param {Array[ interceptorsNAvigationInterceptor]} the collection of actual navigation interceptor instances
 */
 ...
 * </p>
 *
 * @param  getReferenceElement  {FunctionA callback to get the Popup element
 *
 ...
 * @param  loop {Boolean} True if the tab key must loop through the list, false if the navigation must stop at the edges.
 *
 * @return {NavigationInterceptor} an instance of NavigationInterceptor
 */
function ElementNavigationInterceptor(element, loop) {
    ...
    var onfocus = {
        fn: function(event) {
            ...
            var navigationInformation = null;

            if (navigationInformation == null) {
                navigationInformation = elementInterceptor.getNavigationInformation(target);
            }
            if (navigationInformation == null) {
                navigationInformation = viewportInterceptor.getNavigationInformation(target);
            }
...
/**
 * Show back all elements around the given element, by applying the same traversal as for hideOthers.
 *
 * @param {HTMLElement} element The reference element
 */
prototype.showOthers = function(element) {
    var self = this;
    traverseSiblingsUntilRootOfBranch(element, function(element) {
        self.show(element);
    });
};
 ...
 *
 * @return {Boolean} Whether or not the element was actually hidden (first request)
 */
...
/**
 * Registers the request of showing the given element by decreasing the number of hiding requests, and actually shows it back if there are no more such requests.
 *
 * <p>
 * The current implementation uses an attribute "data-hide-requests-count" to store the number of requests.
 * </p>
 ...
 *
 * @return {Boolean} Whether or not the element was actually shown (when no more hiding requests)
 */
prototype.show = function(element) {
    // ----------------------------------------------------------- destructuring
